 ...
 * @since: 2025/03/04
 */
@Tag(name = "三方-萤石", description = "三方-萤石")
@Slf4j
@RestController
...
public class YingshiApiController {
    ...
    @Resource
    private YsAlarmQueueProcessor ysAlarmQueueProcessor;
    ...
    private void handleAlarm(WebhookMessage finalReceiveMessage) {
        try {
            ...
            if (!Objects.equals(YingShiConsts.MSG_TYPE_ALARM, finalReceiveMessage.getHeader().getType())) {
                log.debug("非告警信息:{}", finalReceiveMessage.getHeader().getType());
                return;
            }

            // 识别是否有效序列号
            if (!DEVICE_SERIAL_LIST.contains(finalReceiveMessage.getHeader().getDeviceId())) {
                log.debug("无效的序列号:{}", finalReceiveMessage.getHeader().getDeviceId());
                return;
            }
            ...
            if (!YingShiConsts.ALARM_TYPE_LIST.contains(ysAlarmMsg.getAlarmType())) {
                log.debug("无效的告警类型:{}", ysAlarmMsg.getAlarmType());
                return;
            }
            ...
            log.debug("识别有效信息:{}", JSON.toJSONString(ysAlarmMsg));
            ysAlarmQueueProcessor.addTask(ysAlarmMsg);
        } catch (Exception e) {
